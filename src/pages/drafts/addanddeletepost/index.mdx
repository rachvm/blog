...
I was then able to use this information to pass back to the from end and could change the state on my posts so that thenew post could show otherwise the new post wouldn't show until the page rendered
...
The next steps will be to research and set up testing, so as I add more features add in authorisation I can have confidence in my code. This will also allow me to set up testing and explore github protections
so when I have a continuous deployment on my main branch then I know as code is deployed to that branch it will pass all tests.